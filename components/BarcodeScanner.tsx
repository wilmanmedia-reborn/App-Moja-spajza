...
export const BarcodeScanner: React.FC<Props> = ({ onScan, onClose, isAnalyzing }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950 p-4">
      <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] w-full max-w-md overflow-hidden relative shadow-2xl flex flex-col max-h-[85vh]">
        ...
        <div className="p-6 flex justify-between items-center z-[60] shrink-0 border-b border-slate-100 dark:border-white/5">
          <div>
            ...
            <p className="text-[9px] text-emerald-500 font-black uppercase tracking-widest mt-0.5">Vysoká presnosť aktívna</p>
          </div>
          <button onClick={onClose} className="p-3 bg-slate-100 dark:bg-slate-800 text-slate-500 rounded-xl hover:bg-red-50 hover:text-red-500 transition-colors">✕</button>
        </div>
        ...
        <div className="relative aspect-video bg-black overflow-hidden shrink-0">
          ...
          {!isAnalyzing && !error && (
            <div className="absolute inset-0 pointer-events-none flex flex-col items-center justify-center z-50">
              <div className="w-[320px] h-[160px] relative">
                ...
                {/* Červený laser prispôsobený na 160px výšku */}
                <div className="absolute left-4 right-4 h-[2.5px] bg-red-500 shadow-[0_0_15px_red] animate-laser-new"></div>
              </div>
              <div className="mt-8 text-[10px] text-white/40 font-black uppercase tracking-[0.3em] bg-black/40 px-4 py-1.5 rounded-full backdrop-blur-sm">Vycentrujte kód</div>
            </div>
          )}
          ...
          {isAnalyzing && (
            <div className="absolute inset-0 bg-slate-950/90 backdrop-blur-md z-[70] flex flex-col items-center justify-center text-center p-8">
              ...
              <h4 className="text-white font-black text-xl mb-2 uppercase tracking-tight">Vyhľadávam</h4>
              <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest animate-pulse">Sťahujem údaje z databázy...</p>
            </div>
          )}
        </div>
        ...
        <div className="p-6 bg-slate-50 dark:bg-slate-800/30">
          ...
          >
            <input 
              ...
              className="flex-1 px-4 py-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-bold dark:text-white outline-none focus:border-emerald-500 transition-colors"
            />
            <button 
              type="submit"
              disabled={!manualCode.trim() || isAnalyzing}
              className="px-6 bg-slate-900 dark:bg-emerald-600 text-white rounded-xl font-black text-[10px] uppercase shadow-lg disabled:opacity-30 active:scale-95 transition-all"
            >
              OK
            </button>
          </form>
      ...
      <style>{`
        /* SKRYTIE RUŠIVÝCH PRVKOV KNIŽNICE */
        ...
        @keyframes laser-move-new {
          0%, 100% { transform: translateY(0); opacity: 0.3; }
          50% { transform: translateY(158px); opacity: 1; }
        }
        .animate-laser-new {
          animation: laser-move-new 2s ease-in-out infinite;
        }
      `}</style>
    </div>
  );
